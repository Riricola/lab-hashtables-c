...
public class ProbedHashTable<K,V> implements HashTable<K,V> {
   ...
   */
  int find(K key) {
    int potentialIndex = Math.abs(key.hashCode()) % this.pairs.length;
    K result = ((Pair<K,V>) this.pairs[potentialIndex]).key();
    if(this.pairs[potentialIndex] != null && (! result.equals(key))){
      while(this.pairs[potentialIndex] != null && (! result.equals(key))){
        potentialIndex += Math.abs(key.hashCode()) % this.pairs.length;
      }
